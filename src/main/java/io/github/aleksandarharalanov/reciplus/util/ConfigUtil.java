package io.github.aleksandarharalanov.reciplus.util;
...
public class ConfigUtil extends Configuration {
     ...
     * @param fileName the name of the configuration file to manage (e.g., "shaped.yml", "settings.yml")
     */
    ...
    public void load() {
        ...
        try {
            ...
            logger.severe(String.format("[%s] Failed to load shaped '%s': %s", pluginName, configFile.getName(), e.getMessage()));
        }
    }
    ...
    private void createParentDirectories() {
        try {
            ...
            logger.severe(String.format("[%s] Failed to create default shaped directory: %s", pluginName, e.getMessage()));
        }
    }
    ...
    private void copyDefaultConfig() {
        ...
        try (InputStream input = getClass().getResourceAsStream(resourcePath)) {
            if (input == null) {
                logger.severe(String.format("[%s] Default shaped '%s' wasn't found.", pluginName, configFile.getName()));
                return;
            }

            Files.copy(input, configFile.toPath());
            logger.info(String.format("[%s] Default shaped '%s' created successfully.", pluginName, configFile.getName()));
        } catch (IOException e) {
            logger.severe(String.format("[%s] Failed to create default shaped '%s': %s", pluginName, configFile.getName(), e.getMessage()));
        }
    }
    ...
    public void saveConfig() {
        try {
            ...
            logger.severe(String.format("[%s] Failed to save shaped '%s': %s", pluginName, configFile.getName(), e.getMessage()));
        }
    }
}